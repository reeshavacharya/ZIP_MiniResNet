...
parser = argparse.ArgumentParser(
    description="Generate Go config from preset and flags."
)
parser.add_argument(
    "--preset",
    choices=["gelu", "selu", "elu", "softmax", "layernorm", "relu"],
    required=True,
    help="Activation preset",
)
parser.add_argument("--instances", type=int, required=True, help="Number of instances")
parser.add_argument(
    "--mode",
    choices=["fig1", "table1/2", "table"],
    required=True,
    help="Evaluation mode flag",
)
parser.add_argument(
    "--proving", type=str2bool, required=True, help="Set proving flag (true or false)"
)
...
if args.table_size is not None and args.table_prime_size is not None:
    ...
    if denom <= 0:
        raise argparse.ArgumentTypeError(
            "--table_prime_size must be >= 2 (denominator > 0)"
        )
...
elif preset in ["elu", "selu", "softmax", "layernorm", "relu"]:
