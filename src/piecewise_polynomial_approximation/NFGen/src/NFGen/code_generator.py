...
def code_generate(km_config, profiler, func, basic_time, code_templet, basic_building_blocks, save_file, nick_name=None, not_check=False, code_language="python"):
    ...
    # profiler.predict returns a numpy array; convert to scalar for
    # comparisons and pretty-printing.
    time_km_arr = profiler.predict(np.array([[m, k]]))
    time_km = float(np.asarray(time_km_arr).ravel()[0])

    time_seq = float(predict_sequential_time(func, basic_time))
