...
# Ensure we import the cloned NFGen repo in this folder, not the
# pip-installed package in the virtualenv.
_ROOT = os.path.dirname(os.path.abspath(__file__))
_NFGEN_SRC = os.path.join(_ROOT, "NFGen", "src")
if _NFGEN_SRC not in sys.path:
    sys.path.insert(0, _NFGEN_SRC)
...
relu_config = {
    "function": relu,  # function config.
    "range": (-10, 10),
    # k_max is the *exclusive* upper bound in NFGen,
    # so it must be > 3 to generate any candidates.
    "k_max": 10,  # allow k = 3..9
    "tol": 1e-6,  # precision config.
    "ms": 1000,  # maximum samples.
    "zero_mask": 1e-6,
    "not_check": True, # skip the checking step. so that polynomial coefficient breakdown can happen
    "n": n,  # <n, f> fixed-point config.
    "f": f,
    "profiler": profiler_file,  # profiler model source file.
    "code_templet": temp.templet_spdz,  # spdz templet.
    "code_language": "python",  # indicating the templet language.
    "config_file": "./relu_approx.py",  # generated code file.
    "time_dict": to.basic_time[platform],  # basic operation time cost.
    "derivative_flag": False,
}
...
def main():
    ...
    elif non_linear_operation == "relu":
        generate_nonlinear_config(relu_config)
